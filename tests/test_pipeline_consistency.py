...
def test_raw(raw_dir: str, manifest_path: str):
    ...
    check("Manifest count matches parquet",
          abs(len(df) - manifest.get("total_records", 0)) <= 1,
          f"parquet={len(df)}, manifest={manifest.get('total_records')}")

    required = {"id", "title", "year", "cited_by_count", "abstract", "authors", "concepts", "references"}
    ...
    check("IDs match OpenAlex format",
    ...
    check("Abstract coverage >= 80%", (df["abstract"].str.len() > 20).mean() >= 0.8,
          f"{(df['abstract'].str.len() > 20).mean():.0%}", warn_only=True)
...
def test_clean(clean_path: str, df_raw: pd.DataFrame, report_path: str):
    ...
    check("Record count > 0", len(df) > 0, f"{len(df)} records")
    check("Clean <= Raw (no inflation)",
          len(df) <= len(df_raw),
          f"clean={len(df)}, raw={len(df_raw)}")
    check("Retention >= 70%",
          len(df) / len(df_raw) >= 0.70,
          f"{len(df)/len(df_raw):.0%}", warn_only=True)

    # IDs must be a strict subset of raw IDs
    raw_ids = set(df_raw["id"])
    clean_ids = set(df["id"])
    check("Clean IDs ⊆ Raw IDs", clean_ids.issubset(raw_ids),
          f"{len(clean_ids - raw_ids)} IDs not in raw")
    check("Clean IDs unique", df["id"].is_unique, f"{df['id'].duplicated().sum()} dupes")

    added_cols = {"has_abstract", "has_concepts", "author_count", "decade",
                  "domain_preliminary", "country_list", "is_international"}
    ...
    check("Decade derived correctly",
          (df["decade"] == (df["year"] // 10 * 10)).all())
    check("Citations non-negative", (df["cited_by_count"] >= 0).all())
    check("author_count >= 0", (df["author_count"] >= 0).all())
    check("Abstract coverage >= 50%",
          df["has_abstract"].mean() >= 0.5,
          f"{df['has_abstract'].mean():.0%}", warn_only=True)
    ...
    if report.get("output_records"):
        check("Report output_records matches parquet",
...
def test_bibliometric(proc_dir: str, df_clean: pd.DataFrame):
    ...
    files = ["publication_trends.json", "citation_stats.json", "top_authors.json",
             "top_journals.json", "top_institutions.json", "concept_landscape.json",
    ...
    if trends_path.exists():
        ...
        check("Trends total_records matches clean",
    ...
    if cit_path.exists():
        ...
        check("Citation n matches clean", cit.get("n") == len(df_clean),
              f"stats.n={cit.get('n')}, clean={len(df_clean)}")
        check("h-index > 0", cit.get("h_index_corpus", 0) > 0,
              f"h={cit.get('h_index_corpus')}", warn_only=True)
        check("g-index >= h-index",
              cit.get("g_index_corpus", 0) >= cit.get("h_index_corpus", 0),
              f"g={cit.get('g_index_corpus')}, h={cit.get('h_index_corpus')}")
        check("Total citations == sum of clean",
    ...
    if auth_path.exists():
        ...
        check("unique_authors > 0", auth.get("unique_authors", 0) > 0,
              f"{auth.get('unique_authors')}")
        top = auth.get("top_by_output", [])
        if top:
            check("Top authors sorted descending",
                  all(top[i]["paper_count"] >= top[i+1]["paper_count"] for i in range(len(top)-1)))
        lotka_a = auth.get("lotka_alpha")
        if lotka_a is not None:
            # α ≈ 2.0 for real data; wider range accepted for small / synthetic corpora
            check("Lotka alpha plausible (0.5–5.0)", 0.5 <= lotka_a <= 5.0,
                  f"α={lotka_a:.3f} (expected ~2.0 for real data)", warn_only=True)
    ...
    if concepts_path.exists():
        ...
        if len(top50) > 1:
            check("Concepts sorted descending",
...
def test_classification(proc_dir: str, df_clean: pd.DataFrame):
    ...
    if stale:
        check("classified IDs match current clean IDs",
              False,
    ...
    check("No null domains", df["domain"].isna().mean() <= 0.05,
          f"{df['domain'].isna().mean():.1%} null")

    valid_subs = {
        "comparative_politics", "political_theory", "electoral_politics", "democratic_theory",
        "radical_right", "latin_american_politics", "european_politics",
...
def test_network(proc_dir: str, net_dir: str, df_classified):
    ...
    for net_name, candidates in {
        ...
        if found:
            ...
            check(f"{net_name} non-empty", G.number_of_nodes() > 0,
                  f"{G.number_of_nodes()} nodes, {G.number_of_edges()} edges")
            check(f"{net_name} no self-loops",
                  nx.number_of_selfloops(G) == 0,
                  f"{nx.number_of_selfloops(G)} self-loops", warn_only=True)
            check(f"{net_name} edge weights >= 0",
    ...
    if cluster_path.exists():
        ...
        check("betweenness_centrality in [0,1]",
        ...
        check("pagerank values > 0",
              (df_cl["pagerank"] > 0).all(), warn_only=True)
        ...
        if df_classified is not None:
            ...
            check("Cluster work_ids ⊆ classified IDs",
                  not orphans,
                  f"{len(orphans)} orphan IDs in clusters not in classified", warn_only=True)

    # Metrics JSON coherence
    if metrics_path.exists():
        metrics = load_js(str(metrics_path))
        check("bibcoupling metrics present", "bibcoupling" in metrics,
              str(list(metrics.keys())), warn_only=True)
        for net in ["bibcoupling", "cocitation", "coauthorship"]:
            if net in metrics:
                m = metrics[net]
                check(f"{net} n_nodes > 0", m.get("n_nodes", 0) > 0,
                      f"n_nodes={m.get('n_nodes')}", warn_only=True)
                check(f"{net} density in [0,1]",
                      0 <= m.get("density", -1) <= 1,
                      f"density={m.get('density')}", warn_only=True)
...
def test_visualization(fig_dir: str, outputs_dir: str):
    ...
    for fname in expected_figs:
        ...
        if path.exists():
            ...
            check(f"{fname} non-trivial (>5 KB)", size_kb > 5,
                  f"{size_kb:.1f} KB", warn_only=True)

    report_path = Path(outputs_dir) / "reports" / "report.html"
    check("HTML report exists", report_path.exists(), str(report_path))
    if report_path.exists():
        html = report_path.read_text()
        check("HTML report references figures",
              "<img" in html, warn_only=True)
...
def test_id_integrity(raw_dir: str, clean_path: str, classified_path: str):
    ...
    if Path(classified_path).exists():
        ...
        check("Classified IDs ⊆ Clean IDs (no stale records)",
              not stale,
              f"{len(stale)} classified IDs absent from current clean",
              warn_only=True)
        check("All clean IDs classified",
              not new_in_clean,
...
def main():
    ...
    df_clean = test_clean(f"{p['data_clean']}/openalex_clean.parquet",
