...
SUBCATEGORY_TO_DOMAIN: Dict[str, str] = {
    sc.split("::")[1]: sc.split("::")[0]
    for sc in SEED_TEXTS.keys()
}
...
class PrototypeStore:
    ...
    def build_from_seeds(self) -> "PrototypeStore":
        ...
        logger.info("Prototype store built: %d centroids, backend=%s",
                    len(self._centroids), self._backend_name)
        return self
    ...
    def update_centroids_from_corpus(
        ...
        for label, label_vec_list in label_vecs.items():
            if len(label_vec_list) < min_samples:
                logger.debug("Skipping centroid update for %s (only %d samples)",
                             label, len(label_vec_list))
                continue
    ...
    def load(self, path: str) -> "PrototypeStore":
        ...
        data = np.load(path)
        self._centroids = {
            k.replace("__", "::"): data[k].astype(np.float32)
            for k in data.files
        }
        meta_path = path.replace(".npz", "_metadata.json")
        ...
        logger.info("Prototype store loaded: %d centroids from %s",
                    len(self._centroids), path)
        return self
