...
REQUIRED_RAW_COLS = {
    "id", "title", "year", "cited_by_count", "authors",
    "concepts", "references", "abstract",
}

REQUIRED_CLEAN_COLS = REQUIRED_RAW_COLS | {
    "has_abstract", "has_concepts", "author_count",
    "domain_preliminary", "decade", "country_list",
}
...
def validate_data(config: dict, stage: str = "D1") -> dict:
    ...
    try:
        ...
        # File existence
        _check(report, "file_exists", path is not None and Path(str(path)).exists(),
               str(path))
        ...
        _check(report, "schema_required_columns",
               len(missing_cols) == 0,
               f"Missing: {missing_cols}" if missing_cols else "OK")

        # ID checks
        _check(report, "id_no_nulls",
               df["id"].notna().all(),
               f"{df['id'].isna().sum()} null IDs")
        _check(report, "id_unique",
               df["id"].is_unique,
               f"{df['id'].duplicated().sum()} duplicates")
        if "id" in df.columns and df["id"].notna().any():
            bad_ids = (~df["id"].str.match(r"^https://openalex\.org/W\d+$", na=False)).sum()
            _check(report, "id_format_openalex",
                   bad_ids == 0,
                   f"{bad_ids} IDs with unexpected format",
                   is_error=False)

        # Title checks
        _check(report, "title_no_nulls",
               df["title"].notna().all(),
        ...
        if df["year"].notna().any():
            ...
            _check(report, "year_valid_range", year_range_ok,
                   f"Year range: {int(df['year'].min())}–{int(df['year'].max())}")

        # Citation checks
        _check(report, "citations_non_negative",
        ...
        if "abstract" in df.columns:
            ...
            _check(report, "abstract_coverage_50pct",
                   abstract_rate >= 0.5,
                   f"Abstract coverage: {abstract_rate:.1%}", is_error=False)
        ...
        if "concepts" in df.columns:
            ...
            _check(report, "concept_coverage_60pct",
                   concept_rate >= 0.6,
                   f"Concept coverage: {concept_rate:.1%}", is_error=False)
        ...
        if Path(manifest_path).exists():
            ...
            if manifest_count:
                ...
                _check(report, "record_count_matches_manifest",
                       diff <= 1,
                       f"DF={len(df)}, manifest={manifest_count}", is_error=False)
    ...
    logger.info("Data Validator [%s]: %s (%d errors, %d warnings)",
                stage, report["status"], len(report["errors"]), len(report["warnings"]))
    return report
...
def validate_statistical(config: dict) -> dict:
    ...
    try:
        ...
        if Path(summary_path).exists():
            ...
            _check(report, "n_records_positive", summary.get("n_records", 0) > 0,
                   str(summary.get("n_records")), is_error=False)
        ...
        if Path(trends_path).exists():
            ...
            if annual:
                ...
                _check(report, "no_extreme_yoy_spike",
                       max_yoy <= 500,
                       f"Max YoY growth: {max_yoy:.1f}%", is_error=False)
        ...
        if Path(authors_path).exists():
            ...
            if lotka_alpha is not None:
                _check(report, "lotka_alpha_plausible",
                       1.0 <= lotka_alpha <= 4.0,
                       f"α={lotka_alpha:.3f} (expected ~2.0)", is_error=False)
            if lotka_r2 is not None:
                _check(report, "lotka_r2_acceptable",
                       lotka_r2 >= 0.6,
                       f"R²={lotka_r2:.3f}", is_error=False)

    except Exception as exc:
        report["warnings"].append(f"Unexpected error: {exc}")

    save_json(report, f"{config['paths']['logs']}/validation_statistical.json")
    logger.info("Statistical Validator: %s (%d warnings)", report["status"], len(report["warnings"]))
    return report
...
def validate_classification(config: dict) -> dict:
    ...
    try:
        ...
        _check(report, "null_domain_under_5pct", null_rate <= 0.05,
               f"Null domain rate: {null_rate:.1%}")

        # Valid domain values
        invalid_domains = set(df["domain"].dropna().unique()) - VALID_DOMAINS
        _check(report, "all_domains_valid", len(invalid_domains) == 0,
               f"Invalid: {invalid_domains}")
        ...
        for _, row in df.iterrows():
            ...
            if domain in VALID_SUBCATEGORIES:
                if sub not in VALID_SUBCATEGORIES[domain]:
                    bad_subs += 1
        _check(report, "all_subcategories_valid", bad_subs == 0,
        ...
        if "domain_source" in df.columns:
            ...
            if len(llm_rows) > 10:
                ...
                _check(report, "llm_confidence_not_uniformly_high",
                       high_conf_rate <= 0.8,
        ...
        _check(report, "other_interdisciplinary_under_40pct",
               other_rate <= 0.4,
               f"Other/interdisciplinary rate: {other_rate:.1%}",
               is_error=False)

        report["domain_distribution"] = df["domain"].value_counts().to_dict()
        report["source_distribution"] = df["domain_source"].value_counts().to_dict() if "domain_source" in df.columns else {}
    ...
    logger.info("Classification Validator: %s (%d errors, %d warnings)",
...
def validate_network(config: dict) -> dict:
    ...
    logger.info("Network Validator: %s (%d errors, %d warnings)",
                report["status"], len(report["errors"]), len(report["warnings"]))
    return report
...
if __name__ == "__main__":
    ...
    parser.add_argument("--validator", required=True,
                        choices=["data", "statistical", "classification", "network"])
